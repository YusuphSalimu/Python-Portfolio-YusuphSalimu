...
This project already includes the compiled `css/style.css`, so you can open `index.html` directly in a browser even if you do **not** have Node.js, npm, or any packages installed.

### Option 1: Open without installing anything

- Open `index.html` directly in your browser, or
- Open the folder in VS Code and use a static preview / Live Server

### Option 2: Run with Node.js installed

If you want local development commands, install Node.js first, then run:
...
# Watch SASS changes and rebuild css/style.css
npm run compile:scss
```

### Updated tooling notes

- `node-sass` was replaced with modern `sass`
- `serve` was added for a simple local browser server
- The site itself is still plain HTML, CSS, and JavaScript
...
If you want to edit the SASS files and regenerate the CSS, run:
...
3. In the 3rd step, you will have to modify the **Site settings and deploy**:
   - **Build command**: `npm run build` if you want Netlify to regenerate CSS during deploy
   - **Publish directory**: `/`

   If you are deploying the project as a plain static site without running npm in your environment, you can also leave the project as-is and publish the repository root because the compiled `css/style.css` is already included.
